...
void init_2x2_feistel_similar_matrix(void* matrix, uint32 n, uint32 t) {
  word (*matrix_ptr)[t] = (word (*)[t]) matrix;
  // Works only for t = 2, i.e., balanced Feistel comparison!
  // 1  1
  // 0  1
  matrix_ptr[0][0] = 0x1;
  matrix_ptr[0][1] = 0x1;
...
int main(int argc, char** argv) {
  ...
  if(argc != 5) {
    std::cout << "Usage: <program> <n> <t> <num_rounds> <num_bits_active>" << std::endl;
    return 1;
  }
  ...
  uint32 matrix_mode = 0; // 0 .. MDS, 1 .. random invertible from K, 2 .. "weak" invertible matrix with many zeros, 3 .. "very weak" invertible matrix (slow diffusion), 4 .. "Feistel-similar" matrix [[1,1], [0,1]], only for t = 2
  ...
  if(cipher_case == 0) {
    ...
    else if(matrix_mode == 4) {
      init_2x2_feistel_similar_matrix((uchar*)data, n, t);
      init_very_weak_invertible_matrix((uchar*)((uchar*)data + (sizeof(word) * t * t)), n, t);
    }
